...
(function() {
  ...
  const CONFIG_STORAGE_KEY = "woLanTransferConfig";
  ...
  function bindUiEvents() {
    ...
    document.querySelectorAll(".theme-btn[data-theme-mode]").forEach((button) => {
  ...
  function setThemeMode(mode) {
    const nextMode = mode === "dark" || mode === "light" || mode === "auto" ? mode : "auto";
    try {
  ...
  function applyThemeMode(mode) {
    ...
    document.querySelectorAll(".theme-btn[data-theme-mode]").forEach((button) => {
      const active = String(button.dataset.themeMode || "") === nextMode;
  ...
  function attachConnection(conn, direction) {
    if (activeConn && activeConn !== conn) {
      ...
      try { activeConn.close(); } catch (error) {}
    }
    ...
    conn.on("open", () => {
      if (!isCurrentConnection(conn, generation)) return;
      setStatus(direction === "incoming" ? "设备已连接" : "已连接到电脑", "connected");
      updateChatComposerState();
  ...
  async function beginIncomingTransfer(conn, meta) {
    ...
    if (completed) {
      safeSend(conn, { type: "file-complete", id: transferId, receivedBytes: completed.receivedBytes });
      return;
    }
  ...
  async function receiveChunk(conn, payload) {
    ...
    try {
      ...
      const expectedSize = seq === transfer.totalChunks - 1
        ? Math.max(0, transfer.size - (seq * CHUNK_SIZE))
        : CHUNK_SIZE;
  ...
  async function finalizeAndConfirmIncoming(conn, transfer) {
    ...
    safeSend(conn, { type: "file-complete", id: transfer.id, receivedBytes: transfer.receivedBytes });
  }
  ...
  async function sendTransfer(conn, transfer) {
    ...
    conn.send({ type: "file-done", id: transfer.id, totalChunks: transfer.totalChunks });
  ...
  function armCompletionProbe(conn, transfer) {
    ...
    const probe = () => {
      ...
      safeSend(conn, { type: "file-done", id: transfer.id, totalChunks: transfer.totalChunks });
  ...
  async function buildGeneratedMobileHtml() {
    const [peerJsSource, htmlTemplate, runtimeTemplate] = await Promise.all([
  ...
  async function loadRuntimeConfig() {
    try {
      ...
      return createRuntimeConfig(stored[CONFIG_STORAGE_KEY]);
    } catch (error) {
  ...
  function isUsableConnection(candidate) {
    return !isPageClosing && activeConn === candidate && Boolean(candidate && candidate.open);
  }
