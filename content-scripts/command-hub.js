...
(function() {
  ...
  const STORAGE_KEYS = {
    ...
    homeOrder: "woCommandHubHomeOrder",
  ...
  function setLayerHtml(layer, html) {
    if (!layer || layer.__webOmniLayerHtml === html) return false;
    layer.innerHTML = html;
    layer.__webOmniLayerHtml = html;
    return true;
  }
  ...
  function runLayerTransition(stage, key, html, direction, scope) {
    ...
    if (record && !record.cancelled) {
      if (key === record.toKey) {
        setLayerHtml(record.toLayer, html);
      ...
      if (key === record.fromKey) {
        setLayerHtml(record.fromLayer, html);
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (Object.prototype.hasOwnProperty.call(changes, STORAGE_KEYS.pins)) {
      ...
      if (!equalStringLists(hubState.store.pins, nextPins)) {
        ...
        hubState.store.personalOrder = sanitizePinnedOrder(hubState.store.personalOrder, nextPins);
        storeChanged = true;
      }
    }
    ...
    if (Object.prototype.hasOwnProperty.call(changes, STORAGE_KEYS.personalOrder)) {
      const nextOrder = sanitizePinnedOrder(readChange(STORAGE_KEYS.personalOrder), hubState.store.pins);
      if (!equalStringLists(hubState.store.personalOrder, nextOrder)) {
        hubState.store.personalOrder = nextOrder;
      }
    }

    if (Object.prototype.hasOwnProperty.call(changes, STORAGE_KEYS.homeOrder)) {
      const nextOrder = sanitizePersonalOrder(readChange(STORAGE_KEYS.homeOrder));
      if (!equalStringLists(hubState.store.homeOrder, nextOrder)) {
        hubState.store.homeOrder = nextOrder;
        storeChanged = true;
      }
    }
  ...
  function sanitizePinnedOrder(raw, pins) {
    const safePins = sanitizePins(pins);
    const pinSet = new Set(safePins);
    const ordered = sanitizePersonalOrder(raw).filter((action) => pinSet.has(action));
    safePins.forEach((action) => {
      if (!ordered.includes(action)) ordered.push(action);
    });
    return ordered.slice(0, PIN_LIMIT);
  }

  function getHomeDisplayOrder() {
    const ordered = sanitizePersonalOrder(hubState.store.homeOrder);
    const orderedSet = new Set(ordered);
    const missingPins = hubState.store.pins.filter((action) => !orderedSet.has(action));
    return sanitizePersonalOrder(missingPins.concat(ordered));
  }
  ...
  async function loadHubStore() {
    try {
      ...
      const pins = sanitizePins(raw[STORAGE_KEYS.pins]);
      const legacyOrder = sanitizePersonalOrder(raw[STORAGE_KEYS.personalOrder]);
      hubState.store = {
        pins,
        recent: sanitizeRecent(raw[STORAGE_KEYS.recent]),
        usage: sanitizeUsage(raw[STORAGE_KEYS.usage]),
        personalOrder: sanitizePinnedOrder(legacyOrder, pins),
        homeOrder: sanitizePersonalOrder(
          raw[STORAGE_KEYS.homeOrder] === undefined ? legacyOrder : raw[STORAGE_KEYS.homeOrder]
        ),
      };
  ...
  async function saveHubStore() {
    try {
      await chrome.storage.local.set({
        ...
        [STORAGE_KEYS.personalOrder]: sanitizePinnedOrder(
          hubState.store.personalOrder,
          hubState.store.pins
        ),
        [STORAGE_KEYS.homeOrder]: hubState.store.homeOrder.slice(0, HOME_PERSONAL_LIMIT * 3),
      });
    } catch (error) {
  ...
  async function togglePin(action) {
    ...
    if (index >= 0) {
      ...
      hubState.store.personalOrder = sanitizePinnedOrder(hubState.store.personalOrder, pins);
      window.webOmniShowToast("已取消收藏", "warn", 1800);
    } else {
      pins.unshift(action);
      hubState.store.pins = Array.from(new Set(pins)).slice(0, PIN_LIMIT);
      hubState.store.personalOrder = sanitizePinnedOrder(
        [action].concat(hubState.store.personalOrder),
        hubState.store.pins
      );
      hubState.store.homeOrder = sanitizePersonalOrder(
        [action].concat(hubState.store.homeOrder.filter((item) => item !== action))
      );
  ...
  function getPersonalCommands(limit, excludeSet) {
    ...
    getHomeDisplayOrder().forEach((action) => {
  ...
  function renderHub(options = {}) {
    ...
    if (options.immediate) {
      ...
      const layer = getInteractiveViewLayer();
      if (layer) setLayerHtml(layer, html);
    }
    renderSummary();
    renderActiveStateSurfaces();
    resetSelection();
  ...
  function persistHomeOrder(list) {
    const visibleOrder = getPersonalActions(list);
    if (visibleOrder.length < 2) return Promise.resolve();

    const visibleSet = new Set(visibleOrder);
    const nextHomeOrder = sanitizePersonalOrder(visibleOrder.concat(
      hubState.store.homeOrder.filter((action) => !visibleSet.has(action))
    ));
    if (equalStringLists(nextHomeOrder, hubState.store.homeOrder)) return Promise.resolve();
    hubState.store.homeOrder = nextHomeOrder;
    return chrome.storage.local.set({ [STORAGE_KEYS.homeOrder]: nextHomeOrder });
  }
  ...
  async function finishPersonalPinDrag() {
    ...
    await persistHomeOrder(drag.list);
    resetSelection();
  }
