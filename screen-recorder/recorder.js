(function() {
  ...
  async function startRecording() {
    if (readOnlyMode || isStarting || mediaRecorder || sessionState.status !== "idle") return;
    ...
    isStarting = true;
    setNotice("正在准备录制，请在浏览器提示中选择共享内容。", "info");
    persistPrefs();
    updateUI();
  ...
  function showLivePreview(stream) {
    ...
    els.previewVideo.parentElement.classList.add("has-preview");
  ...
  function clearPreview() {
    ...
    els.previewVideo.parentElement.classList.remove("has-preview");
  }
  ...
  function updateUI() {
    ...
    const canStart = environmentSupported && !readOnlyMode && idle && !isStarting;
    const canControl = !readOnlyMode && !idle && Boolean(mediaRecorder);

    els.startBtn.disabled = !canStart;
    els.startBtn.textContent = isStarting ? "正在准备…" : "开始录制";
    ...
    const statusState = readOnlyMode
      ? "readonly"
      : isStarting
        ? "starting"
        : idle
          ...
          : paused
            ? "paused"
            : "recording";
    els.statusPill.dataset.state = statusState;
